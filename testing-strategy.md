...
* 在提取可重用的代码块时，为提取出的可复用**方法/模块**加上**小型/中型**测试。
* 在修改遗留代码之前，对覆盖遗留代码的**场景/服务**增加**大型/中型**测试。
...
# <!--fit--> &emsp;小步重构&测试🐾&emsp;

---

# <!--fit--> 1️⃣ **解除耦合** -> 2️⃣ **暴露接缝** -> 3️⃣ **准备激活点** -> 4️⃣ **编写测试** -> 5️⃣ **整理代码**
...
## <!--fit--> 1️⃣ 解除耦合&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;
...
- **核心算法/业务逻辑**：遗留系统中核心算法/业务逻辑一般混杂在其他代码中
    ...
    > 通过常见的“安全”重构将核心代码提取到**独立的类或者方法**中：
    > - 从过长方法或者重复代码中提取方法
    > - 移动方法、移动变量到新的类、将方法移动到基类
    > - 提取基类、提取委托类

    > **架构模式(MV\*)**和**设计模式**中是我们重构时的重要基准
...
## <!--fit--> 2️⃣ 暴露接缝&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;
...
- **端到端场景**和**模块/服务接口**的接缝
    ...
    > 可以读取/写入的测试数据（提供持久化的**抽象接口**，如 ORM）
...
## <!--fit--> 3️⃣ 准备激活点&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;

> 直接或者通过替身构造激活点并设定预期行为

---

- **端到端场景**和**模块/服务接口**的激活点
...
## <!--fit-->4️⃣ 编写测试&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;
...
## <!--fit--> 5️⃣ 整理代码&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;
