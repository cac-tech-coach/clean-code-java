...
> 因为学员人数较多，每一期只能让部分学员（约10人）分享；每一期分享都按照主动报名顺序（只取前10人，截止当期分享开始前均可报名）进行，但我们一定保证全部学员轮转分享一次后，再进行下一次轮转。(例如A学员第一次分享之后，第二次依然第一个主动报名，也必须等到第一次没有分享的同事分享完之后，才会轮到 A）
...
<!-- 1、汪航，重构一个SDK多态的例子
2、马干宣，作业认真，与详细的分析和解决的思路
1)修改日志级别，介绍了自动输出日志和打印模板插件。但debug是否还需要添加if判断debug版本？
2)重构模型
3)添加了注释信息
3、陈蓉，示例相对简单，有写明重构原因
1)添加集合访问限制
...
3)移除双重否定
4、周伍润,临时补充提交的作业
1)简单的if重构代码记录对比，没有说明
5、陈桂鹏，重构了一个布局方法
1)方法抽取重构的例子，有前后的代码对比及说明
6、彭颖，示例比较模糊
1)封装条件，但示例说明比较模糊,不清楚前后的区别
7、刘剑，示例多个步骤，相对完整
1）2个比较完整多个步骤的重构示例，包括类单一职责和方法过大提取
8、马明星
1）策略模式替代条件判断
2）反向判断
3）抽象接口
